...
const CoundownTimer: React.FC = () => {

    const [time, setTime] = useState(0);
    const [isRunning, setIsRunning] = useState(false);
    const[remainingTime, setRemainigTime] = useState(0)

    useEffect(() => {
        let timer: NodeJS.Timeout;
        if (isRunning && remainingTime > 0) {
            timer = setInterval(() => {
                setRemainigTime((prev) => prev - 1)
            }, 1000)
        }
...
<img src="../src/images/A.png" alt="ArEe rAjpUt,,"
className="absolute top-4 right-4 h-auto w-40"
/>


<h1 className="text-4xl font-extrabold uppercase md-6
text-white"> Countdown Time </h1>
...
<div>
    ...
    <footer className=" mt-10 text-black text-sm">
        Under The SuperVision Of Areeba Ashraf
    </footer>
